...
| `ocr.py` | Preprocesses each PNG (flatten alpha → grayscale → upscale → contrast boost; DVD adds 4x upscale + Otsu threshold + quiet-zone border via `preprocess(dvd=True)`), calls Tesseract (`--psm 6 --oem 3`, pipe blacklisted), then repairs leftover tall-glyph misreads (`|`/`¦`, `1`, `[`/`]` → `I`/`l`). |
