...
- **OCR quality:** Tesseract accuracy can degrade on italicized or stylized fonts.
  - *DVD preprocessing:* VobSub strips are tiny and soft, so they get extra cleanup before OCR — a 4× upscale (more serif detail), an adaptive Otsu threshold (preserves thin stems on dim/bright strips), a quiet-zone border (Tesseract reads edge-touching glyphs poorly), and inversion to black-on-white (Tesseract's preferred polarity). PGS (Blu-ray) is already sharp and skips this path.
  - *Misread repair:* the tall, near-identical glyphs `I`, `l`, `1`, `[`/`]`, and `|`/`¦` are the most common bitmap-OCR confusions. Leftover ones are corrected from context (e.g. `]'m` → `I'm`, `wi11` → `will`, `1t` → `It`), while real numbers and sound-cue brackets like `[ Honking ]` are left intact. A review pass is still recommended before treating the output as final.
